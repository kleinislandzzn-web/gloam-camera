...
import {prepareFinish,finishImage,resolveDateSettings} from './finish.mjs?v=20260921-live-measured-1';
...
export class LiveCamera {
 constructor({manifest,video,canvas,getPreset,getSettings,onStatus=()=>{},onState=()=>{},onMetrics=()=>{},mediaDevices=navigator.mediaDevices,secure=window.isSecureContext}){
  ...
  this.assets=new Assets(manifest);this.epoch=0;this.preparation=0;this.frameId=0;this.running=false;this.starting=false;this.facing='environment';this.mirror=false;this.previewSize=720;this.lastFrame=0;this.drawTime=0;this.autoSize=480;this.metricsFrames=0;this.metricsStart=0;this.qualityChanged=0;this.capturing=false;
 ...
 async start(facing=this.facing){
  ...
  try{
   ...
   this.starting=false;this.running=true;this.lastFrame=0;this.drawTime=0;this.autoSize=480;this.metricsFrames=0;this.metricsStart=performance.now();this.qualityChanged=this.metricsStart;this.state();
 ...
 metrics(){
  ...
  if(this.previewSize===0&&now-this.qualityChanged>3000){
   if(this.drawTime>90&&this.autoSize>360){this.autoSize=this.autoSize===720?480:360;this.qualityChanged=now;}
   else if(this.drawTime<35&&this.autoSize<720&&now-this.qualityChanged>10000){this.autoSize=this.autoSize===360?480:720;this.qualityChanged=now;}
  }
 }
 ...
 draw(){
  ...
  this.canvas.dataset.frames=String(Number(this.canvas.dataset.frames||0)+1);this.canvas.dataset.preset=this.snapshot.preset.id;this.canvas.dataset.recipe=this.snapshot.settings.r51Profile||'default';
 }
 async capture(){
